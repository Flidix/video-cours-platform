...
export class VideoService extends DatabaseService{
   ...
   async addVideoToCours(dto: AddVideoToCoursDto, userId: number, video){
      ...
      const toCours = await this.database.courses.findOneOrFail({where: {id: dto.toCoursId, userId}});
      return await this.database.videos.create({...dto, video: videoPath, toCours, fromUser, userId});
   }
   ...
   async getVideoById(videoId: number, userId: number){
      ...
      if (video.userId !== userId) {
         await this.database.buedCourses.findOneOrFail({where: {courseId: video.toCours.id, userId}});
      }
      return video
   }

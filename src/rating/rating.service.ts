...
export class RatingService extends DatabaseService {
   ...
   async calculateAverageRating(coursId: number, stars?: number) {
      const cours = await this.database.courses.findOneOrFail({where: {id: coursId}, relations: {ratings: true}});
      if (cours.ratings.length !== 0) {
         const averageRating =
         Math.floor(cours.ratings.reduce((sum, rating) => sum + rating.stars, 0) / cours.ratings.length)
   ...
   async deleteRating(ratingId: number, userId: number){
      const rating = await this.database.ratings.findOneOrFail({where: {id: ratingId, userId: userId}});
   ...
   async updateRating(dto: UpdateRatingDto, userId: number){
      ...
      const rating = await this.database.ratings.findOneOrFail({where: {id: dto.ratingId, userId: userId}});
      await this.calculateAverageRating(rating.courseId);

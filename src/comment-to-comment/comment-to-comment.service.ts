...
export class CommentToCommentService extends DatabaseService {
   ...
   async deleteComment(commentId: number, userId: number) {
      const candidat = await this.database.commentsToComments.findOneOrFail({where: {id: commentId, userId}});
   ...
   async updateComment(dto: UpdateCommentToCommentDto, userId: number) {
      await this.database.commentsToComments.findOneOrFail({where: {id: dto.commentToCommentId, userId}});
      await this.database.commentsToComments.update({id: dto.commentToCommentId}, {comment: dto.comment});
      return true
   }
